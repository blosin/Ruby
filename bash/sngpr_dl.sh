...
FILE_NAME=$(echo "/usr/src/redmine/files/downloads/proj_$2_issue$3$(date +%s).$1")

curl -v -H $"Content-Type: application/$1" -X GET --data-binary $"@issues.$1" \
	-H $"X-Redmine-API-Key: $API_KEY" http://$IP:4000/projects/$2/issues.$1?query_id=$3 -o $FILE_NAME
