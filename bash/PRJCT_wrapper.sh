...
PROJECT_ID=$(echo $6 | cut -c 4-)

backupCrontab

if [[ -n $( checkCrontab $CALL_DL $FORMAT $PROJECT_ID ) ]]
then
  FILA=$( checkCrontab $CALL_DL $FORMAT $PROJECT_ID | cut -d ":" -f 1 | cut -d " " -f 1 )
  crontab -u redmine -l | sed $"$FILA d"  > text
  writeCrontab $1 $2 $3 $4 $5 $CALL_DL $FORMAT $PROJECT_ID
else
