...
class User < Principal
  ...
  def self.verificarCordinador
    mivar=connection.execute(send(:sanitize_sql_array, "SELECT users.id FROM Users users, Projects projects, Roles roles, Members members, Member_roles member_roles
    WHERE users.id=members.user_id and projects.id=members.project_id and roles.id=member_roles.role_id and members.id=member_roles.member_id and
    projects.name=\'NAOS\' AND roles.name=\'Coordinator\' AND users.id=#{User.current.id}"))
    mivar.to_json
  end
