...
class Issue < ActiveRecord::Base
  ...
  #Devuelve el tiempo de respuesta inicial de una sola issue en estado in progress
  def self.initialResponseInProgress(issue_idmio)
    @mivar= self.find_by_sql("SELECT ((SELECT issue_status_histories.from from ISSUE_STATUS_HISTORIES issue_status_histories, ISSUE_STATUSES issue_statuses WHERE issue_statuses.id=issue_status_histories.status_id AND issue_status_histories.to is null AND issue_id=#{issue_idmio})-(SELECT MIN(issue_status_histories.from) from ISSUE_STATUS_HISTORIES issue_status_histories, ISSUE_STATUSES issue_statuses WHERE issue_statuses.id=issue_status_histories.status_id AND issue_id=#{issue_idmio})) AS description")
    @mivar[0].description
  end  
  #Devuelve el tiempo de respuesta inicial de una sola issue en estado closed
  def self.initialResponseClosed(issue_idmio)
    @mivar2= self.find_by_sql("SELECT ((SELECT MAX(issue_status_histories.from) from ISSUE_STATUS_HISTORIES issue_status_histories, ISSUE_STATUSES issue_statuses WHERE issue_statuses.id=issue_status_histories.status_id AND issue_statuses.name='In Progress' AND issue_id=#{issue_idmio})-(SELECT MIN(issue_status_histories.from) from ISSUE_STATUS_HISTORIES issue_status_histories, ISSUE_STATUSES issue_statuses WHERE issue_statuses.id=issue_status_histories.status_id AND issue_id=#{issue_idmio})) AS description")
    @mivar2[0].description
  end
  #Devuelve el promedio de tiempos de respuesta inicial referidos a un solo proyecto
  def self.initialResponseAvg(project_idmio)
    @mivar3= self.find_by_sql("SELECT AVG(fin.from - inicio.min) AS description FROM (
        SELECT issues.id as miidinicio, issue_status_histories.from
        FROM ISSUES issues, ISSUE_STATUS_HISTORIES issue_status_histories, ISSUE_STATUSES issue_statuses
        WHERE issues.id=issue_status_histories.issue_id AND issue_statuses.id=issue_status_histories.status_id AND issue_status_histories.to is null
        AND issues.status_id=(SELECT issue_statuses.id FROM ISSUE_STATUSES issue_statuses WHERE issue_statuses.name='In Progress') AND project_id=#{project_idmio}
        UNION
          SELECT issues.id as miidinicio, MAX(issue_status_histories.from)
        FROM ISSUES issues, ISSUE_STATUS_HISTORIES issue_status_histories, ISSUE_STATUSES issue_statuses
        WHERE issues.id=issue_status_histories.issue_id AND issue_statuses.id=issue_status_histories.status_id
        AND issues.status_id=(SELECT issue_statuses.id FROM ISSUE_STATUSES issue_statuses WHERE issue_statuses.name='Closed') AND issue_statuses.name='In Progress' AND project_id=#{project_idmio}
        group by issues.id
        ) as fin, (
          SELECT issues.id as miidfin, MIN(issue_status_histories.from)
          FROM ISSUES issues, ISSUE_STATUS_HISTORIES issue_status_histories, ISSUE_STATUSES issue_statuses
          WHERE issues.id=issue_status_histories.issue_id AND issue_statuses.id=issue_status_histories.status_id AND issues.status_id=(SELECT issue_statuses.id FROM ISSUE_STATUSES issue_statuses WHERE issue_statuses.name='In Progress') AND project_id=#{project_idmio}
          group by issues.id
          UNION
          SELECT issues.id as miidfin, MIN(issue_status_histories.from)
          FROM ISSUES issues, ISSUE_STATUS_HISTORIES issue_status_histories, ISSUE_STATUSES issue_statuses
          WHERE issues.id=issue_status_histories.issue_id AND issue_statuses.id=issue_status_histories.status_id AND issues.status_id=(SELECT issue_statuses.id FROM ISSUE_STATUSES issue_statuses WHERE issue_statuses.name='Closed') AND project_id=#{project_idmio}
          group by issues.id
        ) as inicio
        WHERE miidinicio=miidfin
        ")
      @mivar3[0].description
  end
